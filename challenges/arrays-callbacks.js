...
name in front of the habitats. Populate the displayNames array with only the animal_name 
and scientific_name of each animal. displayNames will be an array of strings, and each string should 
follow this pattern: "Name: Jackal, asiatic, Scientific: Canis aureus."

*/
const displayNames = [];
function zooFunc(zooAnimals){
  displayNames.push(zooAnimals.animal_name, zooAnimals.scientific_name);
};
zooAnimals.forEach(zooFunc);
...
function nameFunc(zooAnimals){
  return zooAnimals.animal_name.toLowerCase();
};
const lowCaseAnimalNames = zooAnimals.map(nameFunc);
...
Find the total population from all the zoos using the .reduce() method. Remember the reduce method 
takes two arguments: a callback (which itself takes two args), and an initial value for the count.

*/
let populationTotal = zooAnimals.reduce(function (acc, curr) {
